...
### 2.3 trust chip 카피 (개별 검증 필요 — TBD)

- ⚡ `평일 10분 내 회신`
- ⭐ `누적 N건+` (실제 누적 건수 사장님께 확인 필요)
- 🏝 `제주 전 지역 출장`

**오픈 이슈**: "누적 N건+" 수치가 검증 가능해야 한다. 사장님 확인 전까지는 이 chip을 빼고 나머지 2개만 노출하거나, "프리미엄 셰프 BBQ" 같은 비수치 카피로 대체.
...
2. IP 기반 rate limit — 기존 `RateLimiter`. 새 bucket `'quick-quote'`, **5건/일**.
...
- Turnstile 동일 적용.
- Rate limit: IP당 5건/일 (게시판 인콰이어리는 10건/일이지만 빠른 견적은 더 엄격).
...
1. **trust chip "누적 N건+"의 N 값** — 사장님이 검증 가능한 실제 수치가 있는지 확인 후 결정. 없으면 chip 자체 제거 또는 다른 카피로 교체.
2. **모바일에서 이메일 채널을 숨기는 결정** — 데스크탑에선 노출, 모바일에선 생략하기로 했으나 추후 분석 결과에 따라 조정 가능.
3. **rate limit 5건/일이 적정한가** — 출시 후 abuse 패턴 보고 조정.
